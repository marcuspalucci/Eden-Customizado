...
export const Header: React.FC<HeaderProps> = ({
  ...
  const handleGoToVerse = () => {
    ...
    const verseNumber = parseInt(verseInput);
    if (isNaN(verseNumber) || verseNumber < 1) return;

    // Buscar elemento com data-verse
    const verseElement = document.querySelector(`[data-verse="${verseNumber}"]`);

    if (verseElement) {
      verseElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
      // Destacar temporariamente com amarelo
      verseElement.classList.add('bg-yellow-300', 'px-1', 'rounded');
      setTimeout(() => {
        verseElement.classList.remove('bg-yellow-300', 'px-1', 'rounded');
      }, 2500);
    }
  };
