      ...
      {fullScreenImage && (
        ...
        >
          ...
          <a
            ...
            download={`eden_bible_visual_${Date.now()}.png`}
            onClick={(e) => e.stopPropagation()}
            className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-white/10 hover:bg-white/20 backdrop-blur text-white px-6 py-3 rounded-full flex items-center gap-2 transition-all border border-white/20 shadow-lg group"
          >
            <i className="fas fa-download group-hover:scale-110 transition-transform"></i>
            <span className="font-bold text-sm tracking-wide">BAIXAR IMAGEM</span>
          </a>
        </div>
      )}
    </div>
